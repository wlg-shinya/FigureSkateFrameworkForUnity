...
namespace Wlg.FigureSkate.Tests.Fact
{
    public class QueryTest
    {
        ...
        public async Task Verify()
        {
            ...
            {
                foreach (var classId in classObjectAll2024_25.Select(x => x.data.id))
                {
                    ...
                    {
                        var programObject = ProgramObjectQuery.ById(programObjectAll2024_25, programId);
                        var regulationObject = ProgramComponentRegulationObjectQuery.ById(programComponentRegulationObjectAll2024_25, programObject.data.programComponentRegulationId);

                        ProgramComponent[] components = null;

                        // プログラム構成データが不正なら不整合があるはず
                        Assert.IsFalse(ProgramComponentQuery.Verify(components, regulationObject.data, elementPlaceableSetObjectAll, elementPlaceableObjectAll, elementBaseValueObjectAll2024_25));

                        // 正しく作成された未設定のプログラム構成は特に不整合はないはず
                        components = ProgramComponentQuery.Create(regulationObject.data, elementPlaceableSetObjectAll);
                        Assert.IsTrue(ProgramComponentQuery.Verify(components, regulationObject.data, elementPlaceableSetObjectAll, elementPlaceableObjectAll, elementBaseValueObjectAll2024_25));
